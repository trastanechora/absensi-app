...
const AppProfilePage = () => {
  ...
  const [approvalList, setApprovalList] = useState<
    { name: string; users: { id: string; name: string }[] }[]
  >([]);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (values.dateStart === null || values.dateEnd === null) {
      dispatch({
        ...
        payload: { message: "Mohon isi tanggal cuti", severity: "error" },
      });
      return;
    }
    ...
    values.approvers.forEach((approverList, index) => {
      if (index === 0) {
        approverList.map((approver: { id: string }) =>
          acknolwledgeIds.push(approver.id)
        );
      } else {
        approverList.map((approver: { id: string }) =>
    ...
    fetch("/api/leave", {
      ...
      if (res.status === 200) {
        dispatch({
          type: "OPEN_NOTIFICATION",
          payload: { message: `Pengajuan cuti berhasil`, severity: "success" },
        });
        setTimeout(() => {
          router.push("/app");
        }, 2000);
        ...
        dispatch({
          type: "OPEN_NOTIFICATION",
          payload: {
            message: `Gagal mengajukan cuti, Error: ${error}`,
            severity: "error",
          },
        });
      }
  ...
  const handleOnApproval = useCallback(() => {
    if (selectedApproval.current.id) {
      // @ts-ignore
      fetch(`/api/approval/${selectedApproval.current.id}`, {
        method: "PUT",
        body: JSON.stringify({
          status: "approved",
        }),
        ...
        .then(() => {
          ...
          dispatch({
            type: "OPEN_NOTIFICATION",
            payload: {
              message: `Berhasil menyetujui ijin cuti untuk ${selectedApproval.current.title}`,
              severity: "success",
            },
          });
    ...
    fetch(`/api/approval/list`)
      ...
      .then((responseObject) => {
        ...
        responseObject.data.forEach(
          (
            approver: { users: { id: string; name: string }[] },
            index: number
          ) => {
            if (index !== 0) {
  ...
  const activeNotificationCount = listApprovalRef.current.filter(
    (approval: { status: string }) => approval.status === "pending"
  ).length;
      ...
      <main>
        ...
        >
          <Accordion>
            ...
            <AccordionDetails>
              ...
              >
                ...
                >
                  <Box
                    sx={{ width: "100%", display: "flex", maxWidth: "none" }}
                  >
                    ...
                    <Box sx={{ width: "100%", paddingLeft: 1 }}>
                      <FormControl fullWidth>
                        <DatePicker
                          ...
                          maxDate={dayjs(values.dateStart)
                            .set("month", dayjs(values.dateStart).month() + 1)
                            .set("date", 7)}
                          onChange={(value) =>
                            handleValueChange("dateEnd")({
                              target: {
                  ...
                  return (
                    ...
                    >
                      <Autocomplete
                        ...
                        renderInput={(params) => (
                          <TextField {...params} label={approval.name} />
                        )}
                      />
                    </Container>
                  );
          ...
          <Accordion>
            ...
            <AccordionDetails>
              <Box sx={{ mt: 1 }}>
                {listApprovalRef.current.length === 0 && !isLoading && (
                  <Alert severity="info">Tidak ada permintaan ijin cuti</Alert>
                )}
                {listApprovalRef.current.map((approval: Approval, index) => {
                  ...
                  const secondaryText = `${convertDateToLocaleString(
                    dateStart
                  )} - ${convertDateToLocaleString(dateEnd)}`;
          ...
          <Accordion>
            ...
            <AccordionDetails>
              <Box sx={{ mt: 1 }}>
                {listLeaveRef.current.length === 0 && !isLoading && (
                  <Alert severity="info">Belum ada riwayat cuti</Alert>
                )}
                      ...
                      <ListItemButton
                        onClick={() => {
                          selectedLeave.current = {
                            ...leave,
                            formattedDate: secondaryText,
                            formattedDay: `${leave.dayCount} hari`,
                          };
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              Anda menyetujui untuk memberi ijin cuti{" "}
              {selectedApproval.current.title}?
            </DialogContentText>
          </DialogContent>
          <DialogActions>
            <Button onClick={() => setIsOpenApprovalDialog(false)}>
              Tidak
            </Button>
        ...
        <Dialog
          ...
          aria-describedby="alert-dialog-description"
        >
          <DialogTitle id="alert-dialog-title">Info Cuti Anda</DialogTitle>
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              <Container
                maxWidth={false}
              ...
              >
                ...
                <Box sx={{ width: "100%" }}>{selectedLeave.current.status}</Box>
              </Container>
              <Container
                maxWidth={false}
                disableGutters
                    ...
                    {selectedLeave.current.approvals?.map((approval) => {
                      ...
                      return (
                        <Step key={approval.id}>
                          ...
                          >
                            {approval.user.name}{" "}
                            {approval.type === "acknoledge" ? "(Opsional)" : ""}
                          </StepLabel>
                        </Step>
                      );
                    })}
