...
import { TABLE_HEADER, initialFilterState } from '@/entity/constant/division';
...
const DivisionPage = () => {
  ...
  const handleTriggerAction = (type: string, rowData: any) => {
    if (type === 'view') {
      router.push(`/dashboard/division/${rowData.row.id}`);
    } else if (type === 'edit') {
      router.push(`/dashboard/division/${rowData.row.id}/edit`);
    } else {
  ...
  const handleOnDelete = useCallback(() => {
    if (selectedRow !== null) {
      // @ts-ignore
      fetch(`/api/division/${selectedRow.id}`, { method: 'DELETE' })
        .then((res) => res.json())
        .then(() => {
          setDeleteDialogOpen(false);
          // @ts-ignore
          dispatch({ type: 'OPEN_NOTIFICATION', payload: { message: `Berhasil menghapus departemen dengan nama ${selectedRow?.row?.name}`, severity: 'success' } });
          setSelectedRow(null);
  ...
  return (
    <div className={styles.container}>
      <Head>
        <title>List Departemen | WASKITA - ABIPRAYA JO | Sistem Manajemen Absensi</title>
      ...
      <main className={styles.main}>
        <Container maxWidth={false} disableGutters sx={{ width: '100%', display: 'flex', marginBottom: 1 }}>
          ...
          <Box sx={{ width: '30%', display: 'flex', justifyContent: 'flex-end', alignSelf: 'center' }}>
            <Button variant="contained" onClick={() => router.push('/dashboard/division/add')} disabled={isLoading} sx={{ textTransform: 'none' }}>Tambahkan Departemen</Button>
          </Box>
        </Container>
                ...
                <Container maxWidth={false} disableGutters sx={{ width: '100%', display: 'flex', marginBottom: 1 }}>
                  <Box sx={{ width: '100%' }}>
                    <FormControl fullWidth>
                      <TextField fullWidth id="search-keyword" label="Nama Departemen" variant="outlined" value={values.name} onChange={handleFilterChange('name')} />
                    </FormControl>
                  </Box>
                </Container>
              </Container>
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              {/* @ts-ignore */}
              Data akan hilang dan tidak dapat dikembalikan setelah dihapus. Departemen dengan nama <i style={{ fontWeight: 800 }}>{selectedRow?.row?.name}</i> akan dihapus?
            </DialogContentText>
          </DialogContent>
          <DialogActions>
