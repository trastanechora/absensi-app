...
const PresenceDetailPage = ({ params }: { params: { id: string } }) => {
  ...
  return (
    <div className={styles.container}>
      <Head>
        <title>
          Detail Cuti | WASKITA - ABIPRAYA JO | Sistem Manajemen Absensi
        </title>
      ...
      <main className={styles.main}>
        ...
        >
          ...
          <Box sx={{ width: "100%" }}>
            {detail.dateStart === detail.dateEnd &&
              convertDateToLocaleString(new Date(detail.dateStart))}
            {detail.dateStart !== detail.dateEnd &&
              `${convertDateToLocaleString(
                new Date(detail.dateStart)
              )} - ${convertDateToLocaleString(new Date(detail.dateEnd))}`}
          </Box>
        </Container>
        <Container
          maxWidth={false}
        ...
        >
          ...
          <Box sx={{ width: "100%" }}>{detail.dayCount}</Box>
        </Container>
        <Container
          maxWidth={false}
          disableGutters
        ...
        >
          ...
          <Box sx={{ width: "100%" }}>{detail.status}</Box>
        </Container>
        <Container
          maxWidth={false}
          disableGutters
